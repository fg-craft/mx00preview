# mx00preview

## 🎹What is mx00?
mx00 is a prototype VSTi currently in development, utilizing common Phase Modulation. 

It uses the JUCE framework.
- Up to 8 oscillators available per note.
- Oscillators can use arbitrary waveforms per cycle, plus Soundfont and external input.(Similar to the YAMAHA SY99)
- Multi-stage Envelope Generator, LFO, Keyboard/Velocity Scaling, Modulation Matrix, FILTER, PAN, and FX available per oscillator.
- The modulation matrix will allow specifying PM(FM), AM, and RM per oscillator.

## Progress, or Rather, My Motivation
- The PM engine is complete.
- Oscillator settings are now available.
- PAN settings are now available.
- Modulation Matrix is now available.
- Multi-stage Envelope Generator is now available.
- Implementing the GUI is overwhelmingly more difficult than implementing the modulation engine.
- Considering how to port AWM2 WAVEFORMS. Maybe I'll create an auto-sampling tool and try recording from the actual hardware.

<img src="https://github.com/fg-craft/mx00preview/blob/main/images/2026-02-24 174331.png" alt="2026-02-24 174331" width="300">
<img src="https://github.com/fg-craft/mx00preview/blob/main/images/2026-02-14 171236.png" alt="2026-02-14 171236" width="300">
<img src="https://github.com/fg-craft/mx00preview/blob/main/images/20260117_1.png" alt="サンプル画像20260117-1" width="300">
<img src="https://github.com/fg-craft/mx00preview/blob/main/images/20260117_1.png" alt="サンプル画像20260117-1" width="300">
<img src="https://github.com/fg-craft/mx00preview/blob/main/images/20260117_2.png" alt="サンプル画像20260117-2" width="300">
<img src="https://github.com/fg-craft/mx00preview/blob/main/images/20260117_3.png" alt="サンプル画像20260117-3" width="300">
<img src="https://github.com/fg-craft/mx00preview/blob/main/images/20260117_4.png" alt="サンプル画像20260117-4" width="300">
<img src="https://github.com/fg-craft/mx00preview/blob/main/images/20260117_5.png" alt="サンプル画像20260117-5" width="300">
<img src="https://github.com/fg-craft/mx00preview/blob/main/images/20260117_6.png" alt="サンプル画像20260117-6" width="300">
